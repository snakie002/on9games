---
title: "Neon Abyss 2 確認10月8日脫離搶先體驗 | 同步登陸 PS5、Xbox Series、Switch 及 PC | 支援4人連線合作"
description: "Veewo Games 開發的賽博神話 Roguelike 動作遊戲 Neon Abyss 2 將於 2026 年 10 月 8 日正式發售，同步登陸 PS5、Xbox Series X|S、Nintendo Switch 及 PC Steam，支援最多 4 人連線合作，首日加入 Xbox Game Pass。"
coverImage: post_assets/thumbnail.jpg
permalink: /26/260924-Neon_Abyss_2/index.html
date: 2026-09-24
tags:
  - "Neon Abyss 2"
  - Roguelike
  - Veewo Games
  - 新作情報
  - indie
  - 搶先體驗
categories:
  - sp
  - news
---

# Neon Abyss 2 確認10月8日脫離搶先體驗

Veewo Games 開發、Kepler Ghost 發行的賽博神話 Roguelike 動作遊戲 **Neon Abyss 2**，確認將於 **2026 年 10 月 8 日**正式發售，結束長逾一年的 Steam 搶先體驗階段。本作將同步登陸 PlayStation 5、Xbox Series X|S、Nintendo Switch 以及 PC（Steam）平台，並於首日加入 Xbox Game Pass。

{% gameImage "header.jpg", "Neon Abyss 2 主視覺", 460, 215, "Neon Abyss 2 官方 Steam 主視覺。" %}

## 官方預告片

本片由 PlayStation 官方 YouTube 頻道發佈，為 Neon Abyss 2 首次公開的 PS5 版預告片：

{% lazyYouTube "EOp2wAAlNYQ", "Neon Abyss 2 — Reveal Trailer | PS5 Games" %}

## 遊戲概覽

《Neon Abyss 2》是一款橫向捲軸的 Roguelike 動作遊戲，以「賽博神話」（Cybermyth）為主題，融合了高速戰鬥、道具連鎖（Synergy）系統與隨機生成的關卡結構。玩家在每次挑戰中需要透過不同的戰鬥方式與道具組合，打造獨特的 Build 來應對層出不窮的敵人與頭目。

{% gameImage "screenshot_01.jpg", "Neon Abyss 2 遊戲截圖 — 高速戰鬥場面", 1920, 1080, "官方 Steam 遊戲截圖：賽博主題的高速戰鬥場面。" %}

本作最初於 2025 年 7 月 17 日在 Steam 推出搶先體驗版。經過一年多的持續更新與調整，開發團隊已在搶先體驗期間加入了大量新內容，包括多種戰鬥方式、擴展的道具系統以及新的關卡設計。

{% gameImage "screenshot_02.jpg", "Neon Abyss 2 遊戲截圖 — 合作模式與道具展示", 1920, 1080, "官方 Steam 遊戲截圖：合作模式下的戰鬥與道具展示。" %}

## 1.0 版本重點

正式發售版本的最大亮點之一是 **最多 4 人連線合作模式**。玩家可以與朋友一起進入深淵，協力挑戰關卡中的敵人與頭目。合作模式下的道具分配與戰術配合，勢必為遊戲帶來不同於單人遊玩的策略層面。

除了合作模式之外，1.0 版本還預計帶來更多的戰鬥方式、道具連鎖組合以及遊戲平衡調整。在搶先體驗期間累積的玩家回饋，也已反映在多次更新的內容之中。

## 平台與發售資訊

| 項目 | 內容 |
| --- | --- |
| 開發商 | Veewo Games |
| 發行商 | Kepler Ghost |
| 發售日 | 2026 年 10 月 8 日 |
| 搶先體驗開始 | 2025 年 7 月 17 日 |
| 類型 | Action、Adventure、Indie、RPG、Roguelike |
| 平台 | PS5、Xbox Series X\|S、Nintendo Switch、PC（Steam） |
| 連線合作 | 最多 4 人（Online Co-op） |
| Xbox Game Pass | 首日加入 |
| 官方網站 | [neonabyss.com](http://neonabyss.com) |

本作目前已在 Steam 獲得「大多好評」的近期評價（過去 30 天內約 83% 好評），整體則為「褒貶不一」（約 68% 好評）。搶先體驗期間的持續更新似乎已逐步改善了玩家的遊玩體驗。

<div class="steam-widget">
<iframe src="https://store.steampowered.com/widget/2235200/" loading="lazy" style="display:block; width:100%; max-width:640px; height:190px; border:0;"></iframe>
</div>

官方 Steam 頁面：[Neon Abyss 2](https://store.steampowered.com/app/2235200/)

## 編輯觀點

Neon Abyss 2 在搶先體驗一年多後選擇 10 月 8 日作為正式發售日，時間點上正好趕上年末遊戲旺季。4 人連線合作的加入對一款 Roguelike 來說是個蠻有吸引力的賣點——這類遊戲最怕的就是「一個人刷到膩」，多人合作剛好能延長遊戲壽命。加上首日登陸 Xbox Game Pass，等於把進入門檻降到幾乎為零，對還沒接觸過這系列的玩家來說是個不錯的切入點。

不過要注意的是，目前 Steam 上的整體評價仍然停留在「褒貶不一」，搶先體驗初期的負評影響仍在。正式版能否真正扭轉口碑，還是要看 1.0 的內容量與最佳化程度。